...
exports.getDashboardData = async (req, res) => {
    try {
        ...
        const latestSyncedAsset = await internalSequelize.sequelize
            .query(
                'SELECT * FROM synced_assets ORDER BY created_at DESC LIMIT 1',
        ...
        const updatesLast24h = await internalSequelize.sequelize
            .query(
                `SELECT COUNT(*) as count FROM synced_assets WHERE created_at > (NOW() - INTERVAL 24 HOUR)`,
                { type: internalSequelize.Sequelize.QueryTypes.SELECT }
            )
        ...
        const dashboardData = {
            ...
            'Last information ingested': latestSyncedAsset?.created_at
                ? new Date(latestSyncedAsset.created_at).toLocaleString(
                      userLocale,
                      {
                          month: 'short',
                          day: '2-digit',
                ...
                : 'Never',
            'Updates in last 24h':
                (updatesLast24h === 0 ? 'No' : updatesLast24h) + ' new records'
        };
