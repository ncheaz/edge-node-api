...
exports.triggerPipeline = async (file, sessionCookie, kMiningEndpoint, kMiningPipelineId, inputDatasetDBRecord) => {
    try {
        // Create form data
        ...
        let result = await axios.post(`${kMiningEndpoint}/trigger_pipeline`, formData, {
        ...
        if (result.data.message === "DAG triggered") {
            ...
            await datasetService.storePipelineInfo(inputDatasetDBRecord, pipelineId, runId);

            while (true) {
                await wait(1000);

                let pipelineResp = await axios.get(`${kMiningEndpoint}/check-pipeline-status`, {
                    params: {
