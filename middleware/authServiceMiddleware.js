...
exports.authMiddleware = async (req, res, next) => {
    try {
        ...
        if (!sessionCookie) {
            return res.status(401).json({ authenticated: false, message: 'No session cookie found' });
        }

        // Make a request to the Auth service to verify the session
        const authResponse = await axios.get(`${process.env.AUTH_SERVICE_ENDPOINT}/auth/check`, {
            headers: {
